...
class GPT2Model(GPT2PreTrainedModel):
    def __init__(self, config):
        ...
        if config.answer_position_encoding == 'distance':
            self.wae = nn.Embedding(config.n_positions, config.n_embd, padding_idx=0)
        elif config.answer_position_encoding == 'zero_one':
            self.wae = nn.Embedding(2, config.n_embd, padding_idx=0)
    ...
    Return:
        ...
        if answer_position is not None:
            assert hasattr(self, 'wae')
            answer_embeds = self.wae(answer_position)
        else:
            answer_embeds = 0

        hidden_states = inputs_embeds + position_embeds + token_type_embeds + answer_embeds
...
class GPT2QGModel(GPT2PreTrainedModel):
    r"""
    ...
    """
    def __init__(self, config):
        super(GPT2QGModel, self).__init__(config)
        self.transformer = GPT2Model(config)
        self.lm_head = nn.Linear(config.n_embd, config.vocab_size, bias=False)
