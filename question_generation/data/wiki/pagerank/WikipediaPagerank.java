...
public final class WikipediaPagerank {
	...
	private static final File PAGE_ID_TITLE_SQL_FILE = new File("enwiki-20200401-page.sql.gz");           // Original input file
	private static final File PAGE_ID_TITLE_RAW_FILE = new File("wikipedia-pagerank-page-id-title.raw");  // Cache after preprocessing
	
	private static final File PAGE_LINKS_SQL_FILE = new File("enwiki-20200401-pagelinks.sql.gz");   // Original input file
	...
	public static void main(String[] args) throws IOException {
        ...
        */
        PrintWriter out = new PrintWriter(new FileWriter("wiki-20220401.txt", true));
        printTopPages(pr.pageranks, idToTitle, 15000, out);
	}
    ...
    private static void printTopPages(double[] pageranks, Map<Integer,String> titleById, int NUM_PAGES, PrintWriter out) {
		//final int NUM_PAGES = 30;
		double[] sorted = pageranks.clone();
		Arrays.sort(sorted);
		for (int i = 0; i < NUM_PAGES; i++) {
			for (int j = 0; j < sorted.length; j++) {
				if (pageranks[j] == sorted[sorted.length - 1 - i]) {
					out.printf("  %.3f  %s%n", Math.log10(pageranks[j]), titleById.get(j));
					break;
				}
			}
		}
